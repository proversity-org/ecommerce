...
class SignalTests(CourseCatalogTestMixin, TestCase):
    ...
    @httpretty.activate
    def test_disable_switch_reverts_to_lms_receipt_page(self):
        """
        Ensures that disabling the otto_receipt_page switch results in using the LMS receipt url.
        """
        httpretty.register_uri(
            httpretty.GET, get_lms_url('api/credit/v1/providers/ASU'),
            body='{"display_name": "Hogwarts"}',
        ...
        self.assertEqual(
            ...
            'an {platform_name} course.\n'.format(
                ...
                credit_provider='Hogwarts',
                platform_name=get_current_request().site.name,
                receipt_url=get_lms_url('{}?orderNum={}'.format('/commerce/checkout/receipt', order.number))
            )
        )
